use super::{Expression, Parser, token::Token};
use anyhow::{Result, anyhow};

pub fn parse_next_exp(parser: &mut Parser) -> Result<Expression> {
	let mut sign = false;
	...
	loop {
		...
		match &next_token {
			...
			semicolon @ Token::Semicolon => {
				...
				parser.tokens.push(semicolon.clone());
...
pub fn validate_exp_token(current: &Token, next: &Token) -> bool {
	use Token::*;
...
pub fn expected_exp_tokens(current: &Token) -> Vec<Token> {
	use Token::*;
